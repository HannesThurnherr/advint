...
if not load_feature_dict:
    ...
    if load_adv_model:
        with open("feature_browser/top_activations_per_feature_adv_model.pkl", "wb") as f:
            pickle.dump(top_activations_per_feature, f)
    else:
        with open("feature_browser/top_activations_per_feature_def_model.pkl", "wb") as f:
            pickle.dump(top_activations_per_feature, f)
    ...
    if load_adv_model:
        with open("feature_browser/top_activations_per_feature_adv_model.pkl", "rb") as f:
            top_activations_per_feature = pickle.load(f)
    else:
        with open("feature_browser/top_activations_per_feature_def_model.pkl", "rb") as f:
            top_activations_per_feature = pickle.load(f)
...
def get_top_inputs_for_feature(feature_idx, token_ids):
    ...
    for activation_value, (batch_idx, seq_pos) in top_activations:
        ...
        tokens = token_ids[batch_idx, max(0,seq_pos-10 ):seq_pos + 4]  # Assuming seq_pos is the start of the segment you want
        decoded_text = model.to_string(tokens)
        decoded_token = model.to_string(tokens[-4])
        # Append the decoded text with the activation value
        top_inputs_text.append((activation_value, decoded_token, decoded_text))
